...
namespace Hessians {
  ...
  enum Type{
    ...
    GaussNewton_t  ///< Guass Newton approximation
  };
  ...
  /** Note, the formula we normally see for BFGS denotes the inverse
      Hessian approximation.  This is not the inverse, but the true Hessian
      approximation.  The oldY and oldS lists have the same structure as
  ...
  public:
    ...
    /** It's not entirely clear to me what the best implementation for this
        method really is.  In the following implementation, we require an
        additional k work elements where k is the number of stored gradient
        and position differences.  It's possible to reduce this to 1 or 2,
        but we need to compute redundant information.  It's also possible
        to implementation the compact representation, see "Representations
        of quasi-Newton matrices and their use in limited memory methods"
        from Byrd, Nocedal, and Schnabel.  The problem with that algorithm
        is that is requires machinery such as linear system solves that we
        don't current have.  It also works much better with matrices or
        multivectors of data and we don't require the user to provide these
        abstractions. **/
    void operator () (const U& p,U& result) const{
...
/// Preconditioned truncated Conjugate-Gradients
...
namespace TrustRegion{
  ...
  template <class U>
  void getStep( Operator<U,U>& Minv,
                Operator<U,U>& H,
                const U& u,
                const U& g,
                const double delta,
  ...
  template <class U>
  StoppingCondition checkStop( Operator <U,U>& Minv,
                               const U& u,
                               const U& g,
                               const U& g_typ,
                               const double obj_u,
