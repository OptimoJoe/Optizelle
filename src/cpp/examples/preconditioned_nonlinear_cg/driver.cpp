...
// to modify the parameters, look at the file
// preconditioned_nonlinear_cg.param.  Only the Optizelle section is run,
// so copy in different parameter selections into that header in order to
// experiment.  For reference, the optimal solution to the Rosenbrock function
// is (1,1).
...
int main(int argc,char* argv[]){
    ...
    if(argc!=2) {
        std::cerr << "preconditioned_nonlinear_cg <parameters>" << std::endl;
        exit(EXIT_FAILURE);
    }
