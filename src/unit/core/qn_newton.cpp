...
// (1+-sqrt(3)/2,1/2) and we coerce the algorithm to find the solution at
// (1-sqrt(3)/2,1/2) in this setup.
...
int main(int argc,char* argv[]){
    ...
    auto x = std::vector <Real> { 0.133974, 0.5 };
    ...
    setup.dx_n_star.reset(new std::vector <Real> {1-std::sqrt(3.)/2.,0.5});
