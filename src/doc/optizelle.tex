...
\lstdefinestyle{OptizelleOutput}{
    morekeywords={iter,f,x,merit,grad,dx,kry_why,kry_err,kry_iter,ared,pred,
...
The algorithm converged due to: StepSmall
...
\href{https://github.com/open-source-parsers/jsoncpp}{JsonCpp} & Master & No & Parsing of the configuration files and the default archive/restart engine\\\hline
...
\begin{boldlist}
    ...
    \paramitemu
        ...
        {Natural}
        {No}
        {Current number of iterations taken by the truncated Krylov solver when solving the optimality conditions.  We output \textctref{krylov_iter} at each iteration under the label \textctrefalt{kry_iter}.} 
    
    \paramitemu
        {krylov_iter_max}
        {Natural}
        {Yes}
        {Maximum number of iterations taken by the truncated Krylov solver when solving the optimality conditions.} 
    
    \paramitemu
        {krylov_iter_total}
        {Natural}
        {No}
        {Total number of iterations ever taken by the truncated Krylov when solving the optimality conditions.  This gives information about the total amount computational effort taken by Optizelle.  We output \textctref{krylov_iter_total} at each iteration under the label \textctrefalt{kry_iter_total}.}
    
    \paramitemu
        {krylov_orthog_max}
        {Natural}
        {Yes}
        {Numbers of vectors stored and used in the orthogonalization of our truncated Krylov solver.  In order to turn conjugate direction into conjugate gradient, set this to parameter to $1$.  In practice, if memory is available, it may be worthwhile to over orthogonalize.} 
    
    \paramitemu
        {krylov_stop}
        {KrylovStop}
        {No}
        {Reason why the truncated Krylov solver exited when solving the optimality system.  We output \textctref{krylov_stop} at each iteration under the label \textctrefalt{kry_stop}.}
    
    \paramitemu
        {krylov_rel_err}
        {Real}
        {No}
        {Relative error in the solution returned by the truncated Krylov solver when solving the optimality system.  We output \textctref{krylov_rel_err} at each iteration under the label \textctrefalt{kry_err}.}
    
    \paramitemu
        {eps_krylov}
        {Real}
        {Yes}
        {Relative stopping criteria for our truncated Krylov solver.  In truncated CG, when solving the system $Ax=b$ with preconditioner $B$, we use the stopping criteria  $\|B(Ax_k-b)\|\leq \textctref{eps_krylov}\|B(Ax_0-b)\|$.}
    ...
    \paramitemu
        ...
        {Natural}
        {Yes}
        {Number of failed safe-guard steps before exiting the truncated-Krylov method.  We use this exclusively for our inequality constraints.}
    ...
    \paramitemu
        ...
        {Natural}
        {No}
        {Current number of iterations used in the line search.  We use this to determine the amount of computational effort used by Optizelle during the last iteration.   As a note, we output \textctref{linesearch_iter} at each iteration under the label \textctrefalt{ls_iter}.}
...
\begin{boldlist}
    ...
    \outputitemu
        {kry_iter_total}
        {\textctref{krylov_iter_total}}
        {3}
        {Total number of iterations used by the truncated Krylov methods when solving the optimality system.  We typically use this to determine how many Hessian-vector products we've computed over the entire optimization run.}
...
\begin{boldlist}
    ...
    \algorithmitem
        ...
        {By truncated-Krylov method, we mean truncated-CG.  Our version of truncated-CG actually implements the conjugate direction algorithm, but it reduces to truncated-CG when we set \textctref{krylov_orthog_max} to 1.  Historically, Toint and Steihaug give a description of truncated-CG in their respective papers.  For a modern treatment of truncated-CG see Conn, Gould, and Toint's book.
    \begin{itemize}
...
where $L(h(x))^{-1}$ denotes the inverse of the linear operator induced by the Jordan product, $\circ$, which we refer to as \textctref{linv}.  In other words, $h(x)\circ z=L(h(x))z$.  Then, we plug this equation into the first equation and reduce our Newton system to
    ...
    $$
    At this point, we solve the Newton system using our truncated-Krylov methods.  As a note, when using a line-search method that is not Newton-CG, we using a different scheme and instead set
    $$
...
\end{align*}
Note, the last inequality only occurs in constrained problems.  When we enforce these rules depends on the algorithm.  Specifically, trust-region methods enforce these bounds during the truncated-Krylov solve of the optimality conditions.  Since the Krylov solver may violate the inequality bounds periodically throughout the optimality solve, we save the last feasible iterate during the computation.  When we exit, we take the last feasible iterate and step and compute the safeguard search, which satisfies the fraction to the boundary rule above.  In order to prevent too many discarded steps due to the safeguard, we limit the maximum number of infeasible steps that we allow to be \textctref{failed_safeguard_max}.  In a line-search method, we safeguard the step prior to the line search.  Specifically, we shorten \textctref{alpha0} so that the maximum step length taken by the line search does not exceed our fraction to the boundary rule.  Finally, in the inexact composite step SQP method, we also safeguard our quasi-normal step by enforcing the fraction to the boundary rule during the dogleg computation.  In each case, we calculate the distance to the boundary with the user-defined function \textctref{srch}.  In our \textctref{Rm} and \textctref{SQL} vector spaces, we use a closed form formula for linear and second-order cones and the Arnoldi algorithm for semidefinite cones.

        We reduce \textctref{mu} prior to the truncated-Krylov solve for the optimality system and set $\textctref{mu} = \textctref{sigma}\cdot \textctref{mu}$ when one of the following global or local convergence criteria is satisfied
        \begin{enumerate}
        ...
        \begin{align*}
            ...
            \textnormal{gradstep}(x,y,z) =& \left\{
                ...
                \end{array}\right.
        \end{align*}
        and $W$ denotes the projection onto nullspace of $g^\prime(x)$.  In addition, we must converge \textctref{mu_est} locally
        $$
