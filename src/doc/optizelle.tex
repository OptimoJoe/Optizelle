...
\begin{boldlist}
    ...
    \paramitemu
        ...
        {Natural}
        {No}
        {Current number of iterations taken by the truncated Krylov solver, \textctref{krylov_solver}, when solving the optimality conditions.  We output \textctref{krylov_iter} at each iteration under the label \textctrefalt{KryIter}.} 
    ...
    \paramitemu
        ...
        {KrylovStop}
        {No}
        {Reason why the truncated Krylov solver, \textctref{krylov_solver}, exited when solving the optimality system.  We output \textctref{krylov_stop} at each iteration under the label \textctrefalt{kry_stop}.}
    ...
    \paramitemi
        ...
        {Real}
        {No}
        {Typical value for \textctref{mu}.  Generally, we use the value of \textctref{mu_est} at the first iteration.}
    ...
    \paramitemi
        ...
        {Real}
        {No}
        {How much we truncate \textctref{dx} in an interior point method in order to maintain strict feasibility.  When $1.0$, we do not truncate and take a full step.}
    
    \paramitemi
        {alpha_z}
        {Real}
        {No}
        {How much we truncate \textctref{dz} in an interior point method in order to maintain strict feasibility.  When $1.0$, we do not truncate and take a full step.  Note, we only reference this when we set \textctref{ipm} to \hyperref[itm:InteriorPointMethod]{\textct{PrimalDual}}.}
    ...
    \paramitemi
        ...
        \begin{boldlist}
            \ipmitem
                ...
                $$}
        \end{boldlist}
        Here, $\circ$ denotes the Jordan product, \textctref{prod}; $L(h(x))^{-1}$ denotes the inverse of the linear operator induced by the Jordan product, \textctref{linv}; and $e$ denotes the identity element in the pseudo-Euclidean-Jordan algebra, \textctref{id}.  We describe each of these operations further in the section \hyperref[sec:customvector]{\seccustomvector}.  Implicitly, these choices affect the Hessian modification that we apply during the course of the interior point method.  Nevertheless, in almost all cases, we're better off using the primal-dual interior point method.}
    ...
    \paramitemi
        ...
        $$
        Here, $\circ$ denotes the Jordan product, \textctref{prod}; and $e$ denotes the identity element in the pseudo-Euclidean-Jordan algebra, \textctref{id}.  We describe each of these operations further in the section \hyperref[sec:customvector]{\seccustomvector}.  Then, we drive \textctref{mu} toward zero.  How we drive \textctref{mu} toward zero depends on this parameter.  Precisely,
        \begin{boldlist}
        \cstratitem
...
\begin{boldlist}
    ...
    \outputitemi
        ...
        {\textctref{alpha_z}}
        {2}
        {Amount we truncate \textctref{dz} in order to maintain feasibility with respect to the inequality multiplier.  Note, we only reference this when we set \textctref{ipm} to \hyperref[itm:InteriorPointMethod]{\textct{PrimalDual}}.}
...
$$
        z =  \textctref{mu} L(h(x))^{-1} e.
$$
This guarantees that
\begin{enumerate}
    \item $h(x) \circ z = \textctref{mu} e$
