...
\newcommand{\secconstraints}{Define the constraints}
\newcommand{\secpreconditioners}{Define the preconditioners}
...
\begin{enumerate}
    ...
    \item (Optional) \hyperref[sec:constraints]{\secconstraints},
    \item (Optional) \hyperref[sec:preconditioners]{\secpreconditioners},
...
\section{\secpreconditioners}\label{sec:preconditioners}
        ...
        Since Optizelle is fully matrix-free, its performance depends highly on the quality of the preconditioners provided to it by the user.  To that end, there are two places where preconditioning matters:  the Hessian of the objective function and a KKT system that relates to the equality constraints.  Specifically, we benefit when we can define $P:X\rightarrow X$ such that
$$
        P\delta x \approx \nabla^2 f(x)^{-1}\delta x
$$
and $P:Y\rightarrow Y$ such that
$$
        P\delta y \approx (g^\prime(x)g^\prime(x)^*)^{-1}\delta y.
$$
...
$$
        LU=\nabla^2 f(x)
$$
Then, our preconditioner $P:X\rightarrow X$ approximates 
$$
        P\delta x \approx U^{-1}L^{-1} \delta x.
$$
We say approximate because either $U^{-1}$ or $L^{-1}$ may not exist.  In this case, we can just modify our back solve to ignore any variables that cause problems.
...
$$
This gives us the preconditioner
$$
        P\delta x \approx U^{-1}L^{-1} \delta x.
$$
In theory, we can use a Choleski factorization to solve this system.  The problem with this approach is that the Choleski factorization will fail when $g^\prime(x)$ is not full rank.  Generally, we find it easier to fix a failing back solve, as is the case with a QR or LU factorization, than to fix a failing factorization.
...
$$
        ...
        \end{bmatrix}\delta x.
$$
Generally, using Cramer's rule is a bad idea, but we note it works fine on this small example.  Using this formulation, we define our preconditioner to the Hessian with the code 
\begin{boldlist}
    \exampleitem
        {C++}
...
Optizelle::Unconstrained <Real,XX>::State::t(x)
Optizelle::EqualityConstrained <Real,XX,YY>::State::t(x,y)
Optizelle::InequalityConstrained <Real,XX,ZZ>::State::t(x,z)
Optizelle::Constrained <Real,XX,ZZ>::State::t(x,y,z)
\end{filecontents*}

\begin{filecontents*}{state.py}
Optizelle.Unconstrained.State.t(X,msg,x)
Optizelle.EqualityConstrained.State.t(X,Y,msg,x,y)
Optizelle.InequalityConstrained.State.t(X,Y,msg,x,z)
Optizelle.Constrained.State.t(X,Y,Z,msg,x,y,z)
\end{filecontents*}
...
\section{Call the optimiation solver}
