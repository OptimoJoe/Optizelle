...
% Needed for tables with repeated elements.
\usepackage{array}

% Allows us to stop a certain amount of text duplication
\usepackage{collect}

% Allows us to put a frame allow a selection of text, code, whatever
\usepackage{mdframed}

% Allows the create of starred macros
\usepackage{suffix}
...
% Sets an environment for CMake flags.  Code found at:
% https://tex.stackexchange.com/questions/163491/options-for-creating-tables-with-repeated-elements
\newenvironment{cmakelist}
{\par
\centering\begin{longtable}{p{0.15\textwidth}p{0.75\textwidth}}}
{\end{longtable}\par}
...
% Used for labeling and referring to CMake flags
\newcommand{\textctlabel}[1]{\phantomsection\label{itm:#1}\textct{#1}}
\newcommand{\textctref}[1]{\hyperref[itm:#1]{\textct{#1}}}

% Setup an API environment.  As a note, we use medskipamount since
% \baselineskip interferes with something in mdframed and causes us to output
% an infinite number of pages.
\newenvironment{apilist}
{\par
\centering\begin{mdframed}[skipabove=\topsep,skipbelow=\topsep]}
{\end{mdframed}\par}
\newcommand\apiitem[4]{
    \par\begin{tabular}{ll}
        ...
        \textbf{Code}
    \end{tabular}\par
    #4\vspace{\medskipamount}
}
\WithSuffix\DeclareRobustCommand\apiitem*[4]{\apiitem{#1}{#2}{#3}{#4}\vspace{-\medskipamount}}

% Setup an example environment
\newenvironment{examplelist}
    {\par\centering\begin{mdframed}[skipabove=\topsep,skipbelow=\topsep]}
    {\end{mdframed}\par}
\newcommand\exampleitem[2]{
    \par\begin{tabular}{ll}
        ...
        \textbf{Code}
    \end{tabular}\par
    #2\vspace{\medskipamount}
}
\WithSuffix\DeclareRobustCommand\exampleitem*[2]{\exampleitem{#1}{#2}\vspace{-\medskipamount}}
...
\definecollection{col:OptimizationTypes}
\definecollection{col:Unconstrained}
\definecollection{col:EqualityConstrained}
\definecollection{col:InequalityConstrained}
\definecollection{col:Constrained}
\newsavebox{\colImportOptizelleCpp}
\newsavebox{\colImportOptizellePython}
\newsavebox{\colImportOptizelleMatlab}
\definecollection{col:ImportOptizelle}
\definecollection{col:Rm}
\definecollection{col:ScalarValuedFunction}
\definecollection{col:ScalarValuedFunctionExample}
...
\chapter{Introduction}\label{ch:Introduction}
        ...
        \begin{collect}{col:Unconstrained}{}{}
            $\begin{array}[t]{rcl}
                \min\limits_{x\in X} && f(x)
            \end{array}$
        \end{collect}
        \begin{collect}{col:EqualityConstrained}{}{}
            $\begin{array}[t]{rcl}
                \min\limits_{x\in X} && f(x)\\
                \st && g(x)=0
            \end{array}$
        \end{collect}
        \begin{collect}{col:InequalityConstrained}{}{}
            $\begin{array}[t]{rcl}
                \min\limits_{x\in X} && f(x)\\
                \st && h(x)\succeq 0
            \end{array}$
        \end{collect}
        \begin{collect}{col:Constrained}{}{}
            $\begin{array}[t]{rcl}
                \min\limits_{x\in X} && f(x)\\
                \st && g(x)=0\\
                    && h(x)\succeq 0
            \end{array}$
        \end{collect}
        \begin{collect*}{col:OptimizationTypes}{}{}{}{}
            \begin{center}\begin{tabular}{|l|l|}\hline
                \multicolumn{1}{|c|}{\bf Unconstrained} &
                \multicolumn{1}{c|}{\bf Equality Constrained}\\
                \includecollection{col:Unconstrained} &
                \includecollection{col:EqualityConstrained} \\\hline
                \multicolumn{1}{|c|}{\bf Inequality Constrained} &
                \multicolumn{1}{c|}{\bf Constrained}\\
                \includecollection{col:InequalityConstrained} &
                \includecollection{col:Constrained} \\\hline
...
\chapter{Basic API}\label{ch:Basic}
        ...
        \includecollection{col:OptimizationTypes}
...
\begin{lrbox}{\colImportOptizelleCpp}
\begin{lstlisting}[style=C++,linewidth=0.8\textwidth]
...
\begin{lrbox}{\colImportOptizellePython}
\begin{lstlisting}[style=Python,linewidth=0.8\textwidth]
import Optizelle 
...
\begin{lrbox}{\colImportOptizelleMatlab}
\begin{lstlisting}[style=Matlab,linewidth=0.8\textwidth]
global Optizelle;
setupOptizelle();
\end{lstlisting}
\end{lrbox}

\begin{collect*}{col:ImportOptizelle}{}{}{}{}
\begin{examplelist}
    \exampleitem
        {C++}
        {\usebox{\colImportOptizelleCpp}}
    \exampleitem
        {Python}
        {\usebox{\colImportOptizellePython}}
    \exampleitem*
        {MATLAB/Octave}
        {\usebox{\colImportOptizelleMatlab}}
\end{examplelist}
\end{collect*}

\noindent Generally, the module required by the structures below can be determined by the element in \textbf{Structure}.  For example, in Python, the structure \textct{Optizelle.Unconstrained.State.t} requires the module \linebreak\textct{Optizelle.Unconstrained.State}.  In C++, we always require \textct{optizelle/optizelle.h}, but only require \textct{optizelle/json.h} when working with JSON and \textct{optizelle/vspaces.h} when using our default vector spaces such as \textct{Optizelle::Rm} and \textct{Optizelle::SQL}.
...
\begin{align*}
    ...
    h :& X\rightarrow Z.
\end{align*}
Here, the spaces $X$, $Y$, and $Z$ denote vector spaces.  For most problems, these vector spaces just denote $\re^m$ for some value of $m$.  In this case, we use the following built-in vector space:
\begin{collect*}{col:Rm}{}{}{}{}
\begin{apilist}
    \apiitem
        {C++}
        {\textct{Optizelle::Rm}}
        {Use as is}
        {\lstinputlisting[style=C++,linerange=Rm0-Rm1]{@OPTIZELLECPPPATH@/vspaces.h}}
...
\end{apilist}
\end{collect*}
Note, each of the above structures denotes an algebra on the vector-spaces.  For actual vectors, we simply use \textct{std::vector} in C++, NumPy arrays in Python, and column vectors in MATLAB/Octave.  In other words, from a strictly user point of view, we can safely use the above vector spaces, ignoring their implementation details, as long as we use either a \textct{std::vector}, NumPy array, or column vector for our variables.
...
\begin{collect*}{col:ScalarValuedFunction}{}{}{}{}
\begin{apilist}
    \apiitem
        {C++}
        {\textct{Optizelle::ScalarValuedFunction}}
        {Inheritance}
        {\lstinputlisting[style=C++,linerange=ScalarValuedFunction0-ScalarValuedFunction1]{@OPTIZELLECPPPATH@/optizelle.h}}
...
\begin{collect*}{col:ScalarValuedFunctionExample}{}{}{}{}
\begin{examplelist}
    \exampleitem
        {C++}
