...
{\par
\centering\begin{longtable}{|ll|}\hline}
{\end{longtable}\par}
\newcommand\apiitem[4]{%
    \textbf{Language} & #1\\
    \textbf{Structure} & #2\\
...
\section{Notation}
        ...
        Although we work to keep the interface to Optizelle consistent, each programming language we interface to possesses a slightly different semantics.  As such, the suceeding text contain blocks of the form
\begin{apilist}
    \apiitem
        {}
        {}
        ...
        {}
\end{apilist}
\noindent In these blocks, we first specify the \textbf{Language}, which includes C++, Python, and MATLAB/Octave.  Next, we list the name of the \textbf{Structure} under discussion.  In C++ and Python, this generally corresponds to a class whereas in MATLAB/Octave this becomes a struct array.  Note, these names include the complete namespace, module, or member to access the corresponding item.  Then, we denote the \textbf{Interface}, which is either \textit{Inheritance} or \textit{Members present}.  In the case of \textit{Inheritance}, we must create a new class that derives from the specified structure and fills in the missing details.  In the case of \textit{Members present}, we create a new structure that contains all of the necessary elements.  Finally, the \textbf{Code} section contains the actual code that defines the structure under discussion.  This provides the rest of the interface details. 
