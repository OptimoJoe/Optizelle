...
\usepackage{dirtree}
\usepackage{array} % Needed for our tables with repeated elements
...
\newcommand{\cmakelabels}{%
  \ifcase\value{cmakelabelcount}\or
    Flag:\or
    Type:\or
    Default:\or
    Depends on:\or
...
\newcommand{\printcmakelabel}{%
  \stepcounter{cmakelabelcount}%
  \ifnum\value{cmakelabelcount}=9% This needs to be 1+#labels above
    \setcounter{cmakelabelcount}{1}%
  \fi
  \cmakelabels
}
\newcounter{cmakelabelcount}
...
\begin{center}
\begin{longtable}{>{\printcmakelabel}p{0.15\textwidth}p{0.75\textwidth}}
    &\textct{CMAKE_INSTALL_PREFIX}\\
    &\textct{PATH}\\
    &Varies\\
    &None\\
    &None\\
...
\\
    ...
    &None\\
    &No\\
    &C++ compiler specific flags.  Note, we usually require a special flag to
    enable C++11 compilation such as \textct{-std=c++11} on GCC.\\
\\
    &\textct{CMAKE_BUILD_TYPE}\\
    &\textct{STRING}\\
    &None\\
    ...
    \textct{RELEASE} for production libraries.  Set to \textct{DEBUG} to allow
    profiling through utilities such as
...
\\
    ...
    Note, many BLAS and LAPACK libraries such as those from ATLAS benefit from
    OpenMP directives.\\
\\
    &\textct{ENABLE_BUILD_BLAS_AND_LAPACK}\\
    &\textct{BOOL}\\
...
\begin{center}\begin{longtable}{|ll|}\hline
    ...
    \textbf{Code} & \lstinputlisting[style=C++,linerange=ScalarValuedFunction0-ScalarValuedFunction1]{@OPTIZELLECPPPATH@/optizelle.h}\\\hline
    \textbf{Language} & Python\\
    \textbf{Structure} & \textct{Optizelle.ScalarValuedFunction}\\
    \textbf{Interface} & Inheritance\\
    \textbf{Code} & \lstinputlisting[style=Python,linerange=ScalarValuedFunction0-ScalarValuedFunction1]{@OPTIZELLEPYTHONPATH@/__init__.py}\\\hline
    \textbf{Language} & MATLAB/Octave\\
    \textbf{Structure} & \textct{Optizelle.ScalarValuedFunction}\\
    \textbf{Interface} & Members present\\
    \textbf{Code} & \lstinputlisting[style=Matlab,linerange=ScalarValuedFunction0-ScalarValuedFunction1]{@OPTIZELLEMATLABPATH@/setupOptizelle.m}\\\hline
