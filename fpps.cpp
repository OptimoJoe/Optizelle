...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    // 5. xi
    if(nrhs!=5)
	mexErrMsgTxt("The function fpps requires 5 inputs.");
    if(nlhs!=1)
	mexErrMsgTxt("The function fpps requires 1 output.");
    ...
    // Get the direction of differentiation.  Convert it to a vector.
    const double* xi=mxGetPr(prhs[4]);
    vector <double> xi_xi(xi,xi+mxGetM(prhs[4]));
    ...
    op.pps(i-1,yy,eta_eta,xi_xi,result);
