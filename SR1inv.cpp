...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    // 3. p
    if(nrhs!=3)
	mexErrMsgTxt("The function SR1inv requires 3 inputs.");
    if(nlhs!=1)
	mexErrMsgTxt("The function SR1inv requires 1 output.");
    ...
    list <U> work(noldY);
    for(list <U>::iterator v=work.begin(); v!=work.end(); v++)
	v->resize(mp);
    ...
    (Preconditioners::SR1 < vector <double> > (oldY,oldS,work))(p,result);
