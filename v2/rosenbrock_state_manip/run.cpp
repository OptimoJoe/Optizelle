...
struct MyStateManipulator : public peopt::core<MyVS>::StateManipulator {
    typedef peopt::core<MyVS>::State State;
    void operator () (State& state) {
	State::Vars vars;
	State::Reals reals;
	State::Nats nats;
	State::Params params;
	state.release(vars,reals,nats,params);	
	State::checkNames(vars,reals,nats,params);
	state.capture(vars,reals,nats,params);
	state.check();
...
int main(){
    ...
    // Create the state manipulator
    MyStateManipulator smanip;
    ...
    peopt::core<MyVS>::getMin(state,smanip,F,G,H);
