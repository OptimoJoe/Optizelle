...
dnl of that program.
dnl
dnl July 5, 2012: As a minor edit, I changed the behavior to error out if
dnl the mex command could not be found.
dnl
dnl Code:
...
if test "${MEX+set}" != set ; then
    ...
    AC_PATH_PROGS([MEX], $ax_list, __unknown, $MATLAB/bin:$PATH)
    if test "x${MEX}" == "x__unknown"; then
        AC_MSG_ERROR([Can not find the mex command.])
    fi
fi
AC_SUBST([MEX])
